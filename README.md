...
Angelus 是一个本地优先（local-first）的 Agent **控制平面**，面向需要可靠执行过程、明确证据链和长期任务恢复能力的研究与工程工作流。它建立在 [LLMFetcher](llmfetcher/INDEX.md) 之上，把模型调用、工具执行、协作式任务拆分与持久化状态收拢到一个可检查的工作台中。当前发布线为 `v0.3.0-alpha`。
...
| 能力 | 说明 |
| --- | --- |
| 持久化会话 | 保存会话、事件账本、运行状态和兼容性对话投影，重启后可恢复。 |
| 任务计划 | 将目标拆分为可状态化、可更新、可追踪、可绑定到执行作业的嵌套执行单元。 |
| Agent Swarm | 依赖驱动的多 Agent 协调：DAG 调度、split/gather、输入映射、完成路由；执行图可持久化快照与恢复，已结束的 worker 可复活并携带新任务交接。 |
| 实时观测 | 通过 SSE 展示生命周期、工具调用、Trace 与用量账本，事件账本可重放。 |
| Graph / Archive Memory | 结合线性上下文、压缩归档和图检索提供长期记忆。 |
| TLB RAG | 基于 `INDEX.md` 检索树路由到最小充分的知识文件。 |
| 插件系统 | 受权限门禁、完整性校验和命名空间隔离保护的工具、路由、钩子与连接器扩展点。 |
| MCP 工具 | 使用官方 Python `mcp` SDK 连接 stdio、Streamable HTTP 或兼容 SSE 服务，将发现的远端工具直接加入 Agent。 |
| 跨会话记忆 | 显式授权的会话快照、证据、工件与 handoff 交接，带时间线世代管理。 |
