...
- `Agent._retrieve_context_candidates_for_task(...)`: derives temporary task tags, queries compacted summaries plus tag hits, prefers their intersection, falls back to their union, and always preserves a recent active tail so local continuity survives reselection.
...
- `LLMContextHandler.compress_context(...)`: compresses all still-uncompacted raw entries when ids are omitted, or compresses any explicitly selected timeline ids when ids are supplied. The new summary keeps its own timeline id while `source_uuid` and `source_timeline` preserve the flattened original provenance chain across repeated compression.
