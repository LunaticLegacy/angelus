...
| [core.py](core.py#L135) | `AngelusCore.install_signal_handlers` | `None` | `None` | Install SIGINT receipt and start a non-blocking drain helper. |
| [core.py](core.py#L152) | `AngelusCore.drain_signals` | `None` | `bool` | Force-stop pending attempts outside the Python signal handler. |
| [core.py](core.py#L160) | `AngelusCore.receive_sigint` | `None` | `None` | Announce Ctrl+C and immediately force-stop every current attempt. |
| [core.py](core.py#L179) | `AngelusCore.shutdown` | `None` | `None` | Durably stop live attempts before releasing host resources. |
| [core.py](core.py#L209) | `AngelusCore._drain_signal_loop` | `None` | `None` | Poll the signal supervisor until shutdown requests this daemon exit. |
...
|---|---|---|---|---|
| [core.py](core.py#L29) | `AngelusCore` | `state_root: Path \| None, sessions: SessionHandler \| None` | `object` | Compose the one process-local Angelus application. |
