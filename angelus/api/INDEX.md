...
| [runs.py](runs.py#L44) | `_event_resume_offset` | `request: Request, workspace_id: str, session_id: str, after: int, cursor: int \| None` | `int` | Resolve an SSE durable cursor with compatibility precedence. |
| [runs.py](runs.py#L81) | `start_run` | `request: RunRequest` | `dict[str, str]` | Start one Agent or Swarm in a session-owned worker thread. |
| [runs.py](runs.py#L319) | `get_run_status` | `workspace_id: str, session_id: str` | `dict[str, Any]` | Return durable run state and diagnose a worker lost after a restart. |
| [runs.py](runs.py#L368) | `stream_events` | `workspace_id: str, session_id: str, request: Request, after: int, cursor: int \| None` | `StreamingResponse` | Stream durable session events after a chronological log offset. |
| [runs.py](runs.py#L424) | `stop_run` | `workspace_id: str, session_id: str` | `dict[str, bool]` | Request a stop at the next completed model-and-tool boundary. |
| [runs.py](runs.py#L435) | `force_stop_run` | `workspace_id: str, session_id: str` | `dict[str, bool]` | Interrupt an in-flight model request and kill registered tool processes. |
| [runs.py](runs.py#L450) | `steer_run` | `workspace_id: str, session_id: str, request: SteerRequest` | `dict[str, bool]` | Queue a steering message that Agent.run applies at a safe boundary. |
| [sessions.py](sessions.py#L58) | `list_workspaces` | `None` | `dict[str, list[dict[str, str]]]` | List local workspaces available to the browser console. |
| [sessions.py](sessions.py#L63) | `list_sessions` | `None` | `dict[str, list[dict[str, str]]]` | List browser sessions with a compact durable run-status indicator. |
| [sessions.py](sessions.py#L83) | `workspace_root` | `None` | `dict[str, str]` | Return the on-disk state root that owns every browser session. |
