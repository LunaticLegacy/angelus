...
| `CodexAppServerProvider` / `_discover_history` / `export_history` / `_history_path` | Implements fixed, capability-gated Codex discovery, thread/turn control, diff, approval, and subscription operations without generic RPC passthrough. Discovery and direct import first scan date-nested `rollout-*.jsonl` files below `$CODEX_HOME`/`~/.codex`, derive the authoritative session ID from `session_meta`, skip injected environment envelopes when selecting the title/history, and deduplicate the live/final copies of an assistant reply; a failed App Server never blocks CLI-history import. User/assistant text is preserved while tools and reasoning are omitted. | Registered by the external-provider bootstrap; direct import calls `read` then `export_history`; App Server operations are only the fallback/control path and call `CodexAppServerRuntime.call`. |
...
| `ClaudeCodeProvider` / `_importable_transcript_message` / `export_history` | Inspects Claude transcript JSONL read-only, rebuilds only mainline human-visible user/assistant turns while skipping local-command artifacts, metadata, sidechains, synthetic/API-error replies, and empty thinking, then exports the bounded sequence from the configured history root. It runs CLI `stream-json` only for Angelus-owned processes. Discovered external sessions are never attached or controlled. | Registered by `bootstrap_builtin_providers`; direct import calls `read` then `export_history`. |
...
| `angelus.history._agent_turns_page(...)` / `history.projection.transcript_page(...)` | Compatibility facade and incremental event-log transcript pager. When a Coordinator or aggregate view has no native lifecycle turns, the pager falls back to the imported session's formatted `conversation.json` and preserves cursor pagination, so external history renders in the normal chat pane rather than only in Trace. | Existing API/tests call the facade; it delegates to `history.projection.transcript_page`, which calls `_read_session_history` only for the imported-history fallback. |
...
| `_fetcher_for_config` / `_synchronize_swarm_execution_settings` | Builds the connector-authoritative fetcher and applies a newly resolved profile's connector, round, output-token, and context defaults directly to every retained Agent before the next graph run; `0` remains the explicit unlimited-round value. It retains the Swarm topology, TaskBus, Worker identities, and contexts. | `_build_agent` and `api.runs.start_run` call the fetcher factory; the retained Swarm path calls the synchronizer immediately before `AgentSwarm.run`. |
...
| `start_run(request)` — retained Swarm path | Resolves the saved Agent profile and connector at the run boundary, derives Shell/Swarm activation from their permission categories, reuses the retained graph, and synchronizes its next-turn execution defaults without replacing Worker topology or contexts. Missing models return a connector-setup action message. | Calls `ActiveRun.reset_for_next_turn`, conditionally calls `_restore_swarm`/`_build_swarm`, then calls `runtime._synchronize_swarm_execution_settings` and `AgentSwarm.run`; terminal cleanup persists the recovery snapshot. |
...
| `renderToolPermissions(policy)` | Renders readable Chinese category cards with a category master switch plus independently described tool switches. A category change bulk-enables or bulk-disables every contained tool before saving, while a tool change remains independently adjustable; both re-render effective state. | Called by `applyProfile`; sends profile updates through `apiPut(profileUrl(), ...)`. |
