...
| `start_run(request)` — retained Swarm path | Resolves the saved Agent profile and connector at the run boundary, derives Shell/Swarm activation from their permission categories, then reuses an in-memory completed `ActiveRun`/`AgentSwarm` when compatible. Missing models return a connector-setup action message. | Calls `ActiveRun.reset_for_next_turn`, conditionally calls `_restore_swarm`/`_build_swarm`, then calls `AgentSwarm.run`; terminal cleanup persists the recovery snapshot. |
