...
function applyConnector(connector) { ["provider","model","api-url","system-prompt","temperature","max-tokens","max-rounds","max-context-threshold","max-swarm-agents"].forEach(id=>{const key=id.replaceAll("-","_"); if(connector[key] !== undefined) $(id).value=connector[key];}); $("api-key").value=""; $("api-key").placeholder=connector.has_api_key ? "已安全保存；留空以继续使用" : "仅保留在当前浏览器"; $("enable-shell").checked=Boolean(connector.enable_shell); $("enable-swarm").checked=Boolean(connector.enable_swarm); }
...
function connectorPayload(name) { const {connector_id, ...settings}=config(); return {name, ...settings}; }
