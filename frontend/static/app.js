...
function handleEvent(event) {
  durableEventCount+=1;
  if(event.event === "lifecycle") { traceEvents.unshift(event); tracePayload(event); renderAgentSelector(currentAgents); if(selectedAgent === "all" || selectedAgent === event.agent) appendAgentBehavior(event); if(event.type === "agent:complete") updateHeaderMetrics(event.data); if(activeInspectorPanel === "inspector-usage" && event.type === "agent:round") loadUsage().catch(error=>trace("用量加载失败",error.message)); if(activeInspectorPanel === "inspector-agents") loadInspectorAgents().catch(error=>trace("Agent 检查器加载失败",error.message)); if(event.source === "graph" || event.type.includes("task:")) loadGraph().then(loadAgents).catch(error=>trace("执行图加载失败",error.message)); return; }
...
$("composer").addEventListener("submit", (event)=>{event.preventDefault(); const message=$("message").value; if(!message.trim()) return; $("message").value=""; start(message);});
...
$("new-connector").addEventListener("click", async()=>{const name=window.prompt("连接名称");if(!name?.trim())return;const response=await fetch("/api/connectors",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(connectorPayload(name))});const connector=await response.json();if(!response.ok){alert(connector.detail||"无法保存连接");return;}await loadConnectors(connector.id);trace("已保存连接",connector.name);});
$("save-connector").addEventListener("click", async()=>{if(!connectorId){$("new-connector").click();return;}const name=$("connector").selectedOptions[0].text;const response=await fetch(`/api/connectors/${connectorId}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(connectorPayload(name))});if(!response.ok){alert("无法更新连接");return;}trace("已更新连接",name);});
