...
let authToken = sessionStorage.llmfetcherToken || "";
function authHeaders(extra={}) { const h = {...extra}; if(authToken) h["Authorization"] = "Bearer " + authToken; return h; }
function authQuery(url) { if(!authToken) return url; const sep = url.includes("?") ? "&" : "?"; return url + sep + "token=" + encodeURIComponent(authToken); }
function setAuthToken(token) { authToken = (token || "").trim(); if(authToken) sessionStorage.llmfetcherToken = authToken; else sessionStorage.removeItem("llmfetcherToken"); }
function promptForToken() { return new Promise(resolve=>{ const dialog=$("auth-dialog"); const form=$("auth-form"); const input=$("auth-token"); if(!dialog || !form) { resolve(false); return; } let done=false; const finish=(ok)=>{ if(done) return; done=true; form.removeEventListener("submit", onSubmit); $("cancel-auth").removeEventListener("click", onCancel); if(dialog.open) dialog.close(); resolve(ok); }; const onSubmit=(e)=>{ e.preventDefault(); const t=input.value.trim(); if(!t){ input.focus(); return; } setAuthToken(t); finish(true); }; const onCancel=()=>{ finish(false); }; form.addEventListener("submit", onSubmit); $("cancel-auth").addEventListener("click", onCancel); input.value=""; dialog.showModal(); input.focus(); }); }
async function authedFetch(path, options={}) { let opts = {...options, headers: authHeaders(options.headers || {})}; let response = await fetch(path, opts); if(response.status === 401) { if(await promptForToken()) { opts = {...options, headers: authHeaders(options.headers || {})}; response = await fetch(path, opts); } } return response; }
...
function setRunning(running) { $("send").disabled=running; $("stop").disabled=!running; $("force-stop").disabled=!running; $("message").disabled=running; const guidance=$("run-guidance"); if(running && !guidance){const panel=document.createElement("aside");panel.id="run-guidance";panel.className="run-guidance";panel.innerHTML="<strong>Agent 正在执行</strong><span>可在右侧查看工具调用与用量。</span><span>停止会在当前模型与工具步骤完成后生效。</span><span>强行停止会立即终止已注册的 Shell 工具进程。</span><span>切换工作空间不会中断后台任务，结果会保存在原会话。</span>"; $("chat").append(panel);} if(!running) guidance?.remove(); }
async function apiJson(path) { const response=await authedFetch(path); if(!response.ok) throw new Error(`${response.status} ${response.statusText} (${path})`); return response.json(); }
...
function applyConnector(connector) { ["provider","model","api-url","system-prompt","temperature","max-tokens","max-rounds","max-context-threshold","max-swarm-agents"].forEach(id=>{const key=id.replaceAll("-","_"); if(connector[key] !== undefined) $(id).value=connector[key];}); $("api-key").value = connector.api_key_hint || (connector.api_key || ""); $("api-key").placeholder = connector.has_api_key ? "已保存密钥（输入新值可替换）" : "仅保留在当前浏览器"; $("enable-shell").checked=Boolean(connector.enable_shell); $("enable-swarm").checked=Boolean(connector.enable_swarm); }
...
async function updatePlanStatus(taskId,status) { const response=await authedFetch(`${planUrl()}/tasks/${taskId}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({status})}); if(!response.ok) throw new Error("任务状态更新失败"); await loadPlan(); }
...
async function start(message) {
  ...
  try {
    const response=await authedFetch("/api/runs", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({session_id:sessionId, workspace_id:workspaceId, message, config:config()})});
...
function connectRunEvents(runId) { source?.close(); const eventSource=new EventSource(authQuery(`/api/workspaces/${workspaceId}/runs/${runId}/events?after=${durableEventCount}`)); source=eventSource; sourceWorkspaceId=workspaceId; eventSource.onmessage=(event)=>{ if(workspaceId === sourceWorkspaceId) handleEvent(JSON.parse(event.data)); }; eventSource.onerror=()=>{ if(eventSource.readyState===EventSource.CLOSED && source===eventSource) finish(); }; }
...
$("stop").addEventListener("click", async()=>{await authedFetch(`/api/workspaces/${workspaceId}/runs/${sessionId}/stop`,{method:"POST"}); $("stop").disabled=true;setStatus("等待安全停止", "running");});
$("force-stop").addEventListener("click", async()=>{if(!confirm("强行停止当前会话？正在执行的 Shell 工具进程也会被立即终止。"))return; $("force-stop").disabled=true; $("stop").disabled=true; setStatus("正在强行停止", "running"); const response=await authedFetch(`/api/workspaces/${workspaceId}/runs/${sessionId}/force-stop`,{method:"POST"}); if(!response.ok){const message=`${response.status} ${response.statusText}`;trace("强行停止失败",message);appendRunErrorBlock("强行停止失败",message);setStatus("停止失败","error"); setRunning(true); return;} trace("强行停止","已请求终止当前 Agent 及工具进程。");});
...
async function createAndSwitchSession(name) {
  if(!name?.trim()) return;
  try {
    const response=await authedFetch("/api/sessions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name})});
...
$("delete-workspace").addEventListener("click",async()=>{const selected=$("workspace").selectedOptions[0];if(!selected)return;const name=selected.text;if(!confirm(`删除会话“${name}”及其所有数据？此操作不可恢复。`))return;const confirmation=prompt(`请输入会话名称“${name}”以确认删除：`);if(confirmation !== name)return;const response=await authedFetch(`/api/sessions/${sessionId}`,{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({confirmation})});const payload=await response.json();if(response.status===404){await loadWorkspaces();await loadHistory();await loadPlan();await loadGraph();trace("会话已不存在",`${name} 已被移除，已切换到有效会话。`);return;}if(!response.ok){alert(payload.detail||"无法删除会话");return;}if(payload.status==="stopping"){trace("正在停止并删除会话",payload.message);return;}await loadWorkspaces();await loadHistory();await loadPlan();await loadGraph();trace("会话已删除",name);});
...
$("delete-connector").addEventListener("click", async()=>{if(!connectorId||!confirm("删除这个连接及其保存的密钥？"))return;const response=await authedFetch(`/api/connectors/${connectorId}`,{method:"DELETE"});if(!response.ok){alert("无法删除连接");return;}connectorId="";localStorage.llmfetcherConnector="";await loadConnectors();trace("已删除连接");});
